...
// 7. Supported models
List<String> supportedModels = ["QL-1110NWB", "RJ-2150", "PJ-763MFi"];
// 8. Selected Choices
...
class FindPrinters {
  ...
  Future<Null> _discoverPrintersBlt() async {
    ...
    bluetoothManager.startScan(timeout: Duration(seconds: 30));
    bluetoothManager.scanResults.listen((devices) {
      devices.forEach((element) {
        debugPrint("========================================>");
        debugPrint(element.name.toString());
        debugPrint(element.address.toString());
        for (String printr in supportedModels) {
          debugPrint(element.name);
          if (element.name.startsWith(printr)) {
            debugPrint(
                "YAAAAAAAAAY ----------------------------------------!!!!!!!!!!!!!!!!!!!!!!!");
            debugPrint(element.name);
          }
